<script>
	...
	async function openCode(codePath) {
		...
		const githubPath =
			"https://raw.githubusercontent.com/Pesopes/disjuncture/master/" +
			codePath;
		...
		if (
			imageExtensions.includes(
				codePath.slice(codePath.lastIndexOf(".") + 1),
			)
		) {
			openImage(
				githubPath,
	...
	function openNote(noteDate) {
		...
		cache[0].appInfo.selectedNote = $notes.findIndex(
			(el) => el.date === noteDate,
		);
				...
				files: [
					{
						...
						files: [
							{
								name: "100456",
								files: [{ name: "vsjitsjčinkčong.dll" }],
							},
							{
								name: "104251",
								files: [
							...
							{ name: "aticds64.dll" },
							{
								name: "DOnoot_opE_N🤑.dll",
								clickEvent: openError,
							},
							{ name: "mscvcp.dll" },
							{ name: "NPSM.dll" },
							{ name: "nshts.dll" },
						...
						files: [
							{
								name: "pop.ogg",
								clickEvent: () =>
									openCode("assets/sounds/pop.ogg"),
							},
						],
					},
				],
...
<Popup bind:visible bind:isPinned {title}>
		...
		{:then file}
			...
			{:then content}
				<Prism
					...
					code={content}
					header={codeViewer.title.slice(
						codeViewer.title.lastIndexOf("/") + 1,
					)}
				/>
				<a
					style="position: absolute;bottom: 0%;left:50%;"
					href={window.URL.createObjectURL(codeViewer.src)}
					download
				>
...
{#each imageViewers as imageViewer}
	<Popup bind:visible={imageViewer.visible} title={imageViewer.title}>
		<img
			...
			alt="Icon for {imageViewer.source}"
			style="width: 100%;height: 100%;"
		/>
		<a
			style="position: absolute;bottom: 0%;left:50%;"
			href={imageViewer.source}
			download
		>
			<img src={DownloadImage} alt="Download" width="20" />
		</a>
